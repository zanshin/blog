...
class Cat {
    ...
    draw(dt) {
        ...
        const sprite = this.spritesheet.getSprite(this.frame.name, this.frame.frame)
...
function cat(canvas) {
    ...
    // const spritesheet = new Spritesheet(256, 320, 8, 10, animations, '/static/cat/cat.png');
    const spritesheet = new Spritesheet(256, 320, 8, 10, animations, '/js/cat/cat.png');
    const cat = new Cat(spritesheet, canvas);

    spritesheet.load(() => {
        setInterval(function(){cat.update(0.033)}, 33);
    });
}
